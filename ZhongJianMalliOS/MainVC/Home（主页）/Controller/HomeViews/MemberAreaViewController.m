...
@interface MemberAreaViewController ()<UITableViewDelegate,UITableViewDataSource> {
    BOOL selectedPrice;
    BOOL selectedMinite;
    UIView *lineBottom;
...
- (void)createData:(BOOL)isMore {
    ...
    if ([_selectedTag isEqualToString:@""]) {
        //
                NSString *url = [NSString stringWithFormat:@"%@%@",HTTPUrl,@"/zjapp/v1/HomePage/MemberArea"];
                NSDictionary *dic = @{
                                      @"type":selectedType,
                                      @"page":page,
                                      @"pageNum":@"10"
...
- (void)creatTableView {
    ...
    self.memberTable = [[UITableView alloc] initWithFrame:CGRectMake(0, 64+47, KWIDTH, KHEIGHT-64-47) style:UITableViewStylePlain];
    ...
    self.memberTable.tableFooterView = line;
    self.memberTable.separatorStyle = UITableViewCellSeparatorStyleNone;
    //    __weak typeof(self) weakSelf = self;
    self.memberTable.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        // 进入刷新状态后会自动调用这个block
        [self createData:NO];
    }];
    self.memberTable.mj_footer = [MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
        [self createData:YES];
    }];
...
- (void)endRefresh
{
    [_memberTable.mj_header endRefreshing];
    if (is_noMore == YES) {
        [_memberTable.mj_footer endRefreshingWithNoMoreData];
//        _memberTable.mj_footer.hidden = YES;
    }else {
        [_memberTable.mj_footer endRefreshing];
    }
}
/*
