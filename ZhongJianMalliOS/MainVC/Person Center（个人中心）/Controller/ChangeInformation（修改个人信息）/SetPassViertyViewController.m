...
@interface SetPassViertyViewController () {
...
@implementation SetPassViertyViewController
...
- (void)configView {
    ...
    nextButton.tag = 1300;
    [nextButton addTarget:self action:@selector(nextViewButtonClick:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)nextViewButtonClick:(UIButton *)sender {
    
    NSString *url = [NSString stringWithFormat:@"%@%@",HTTPUrl,@"/zjapp/v1/LoginAndRegister/verify/"];
    ...
    [manager POST:url parameters:dic progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if ([code isEqualToString:@"0"]) {
            SetPayPassViewController *paypassVC = [SetPayPassViewController new];
//            paypassVC.
            NSString *code = [NSString stringWithFormat:@"%@",responseObject[@"data"]];
            paypassVC.verifyCode = code;
...
//            [alert addAction:[UIAlertAction actionWithTitle:@"发送成功" style:UIAlertActionStyleDefault handler:nil]];
            ...
            UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"验证码不正确" message:@"" preferredStyle:UIAlertControllerStyleAlert];
